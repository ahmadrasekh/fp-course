...
instance Functor (State s) where
  ...
  (<$>) f sa =
    let stateFunction = runState sa
     in State (\s -> let (a, s') = stateFunction s in (f a, s'))
...
instance Applicative (State s) where
  pure :: a -> State s a
  pure a = State (a,) -- State (\s -> (a, s))
  (<*>) :: State s (a -> b) -> State s a -> State s b
  (<*>) sab sa =
    let x = runState sab
     in let y = runState sa
         in State (\s -> let (f, s') = x s in let (a, s'') = y s' in (f a, s''))
